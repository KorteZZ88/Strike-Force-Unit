...
#include "../../game_shared/weapon_context.h"
...
#include "../../game_shared/weapons/xm1014.h"
...
#include "../../game_shared/weapons/mine_ap.h"
...
#include "../../game_shared/weapons/g3sg1.h"
#include "../../game_shared/weapons/sg550.h"
#include "../../game_shared/weapons/awp.h"
#include "../../game_shared/weapons/aug.h"
#include "../../game_shared/weapons/m60.h"
#include "../../game_shared/weapons/m249.h"
...
void CBombGameRules::PlayerSpawn(CBasePlayer*p){EnsureMoney(p);if(!IsValidTeam(p->TeamID())){p->StartObserver(p->GetAbsOrigin(),p->pev->v_angle);return;}CHalfLifeMultiplay::PlayerSpawn(p);if(!p->HasNamedPlayerItem("weapon_knife"))p->GiveNamedItem("weapon_knife");if(p->m_rgpPlayerItems[2]==NULL){const bool blue=!Q_stricmp(p->TeamID(),BLUE);p->GiveNamedItem(blue?"weapon_beretta":"weapon_glock18");m_weaponPurchased[p->entindex()][blue?WEAPON_BERETTA:WEAPON_GLOCK18]=false;}SetKnifeAsLastItem(p);SendScoreStatus(p,1);}
...
void CBombGameRules::ShowBuyMenu(CBasePlayer*p,int page)
{
	...
	auto Price=[](const char*classname,int fallback){return GetWeaponConfigInt(classname,"buy_price",fallback);};
	if(page==0){keys|=0x1ff;Q_snprintf(menu,sizeof(menu),"Buy Menu  $%d\n\n1. Pistols\n2. Shotguns\n3. Submachineguns\n4. Assault Rifles\n5. Sniper Rifles\n6. Buy Primary ammo\n7. Buy Secondary ammo\n8. Equipment\n9. Mines\n\n0. Exit",m_money[i]);}
	else if(page==1){const bool red=!Q_stricmp(p->TeamID(),RED),blue=!Q_stricmp(p->TeamID(),BLUE);keys|=0x3f;char team3[80]="",team4[80]="";if(red){Q_snprintf(team3,sizeof(team3),"3. Colt 1911 - $%d\n",Price("weapon_1911",400));Q_snprintf(team4,sizeof(team4),"4. P229 - $%d\n",Price("weapon_p229",650));}else if(blue){Q_snprintf(team3,sizeof(team3),"3. USP .45 - $%d\n",Price("weapon_usp",500));Q_snprintf(team4,sizeof(team4),"4. Five-SeveN - $%d\n",Price("weapon_fiveseven",700));}Q_snprintf(menu,sizeof(menu),"Pistols  $%d\n\n1. Glock 18 - $%d\n2. Beretta - $%d\n%s%s5. Desert Eagle .50 AE - $%d\n6. Raging Bull - $%d\n\n0. Exit",m_money[i],Price("weapon_glock18",300),Price("weapon_beretta",400),team3,team4,Price("weapon_deagle",800),Price("weapon_ragingbull",900));}
	else if(page==4){const bool red=!Q_stricmp(p->TeamID(),RED);keys|=0x3f;Q_snprintf(menu,sizeof(menu),"Assault Rifles  $%d\n\n1. %s - $%d\n2. M24 - $%d\n3. %s - $%d\n4. %s - $%d\n5. %s - $%d\n6. AI AWP - $%d\n\n0. Exit",m_money[i],red?"IMI Galil":"FA MAS",red?Price("weapon_galil",1800):Price("weapon_famas",1950),Price("weapon_m24",1800),red?"AK-47":"M4",red?Price("weapon_ak47",2700):Price("weapon_m4",3100),red?"SG 552":"AUG",red?Price("weapon_sg552",3000):Price("weapon_aug",3300),red?"G3 SG1":"SIG SG 550",red?Price("weapon_g3sg1",4700):Price("weapon_sg550",4900),Price("weapon_awp",4750));}
	else if(page==3){const bool red=!Q_stricmp(p->TeamID(),RED);keys|=0xf;char teamWeapons[160],teamLast[80];if(red){Q_snprintf(teamWeapons,sizeof(teamWeapons),"1. Mac-10 - $%d\n2. MP-5 - $%d\n",Price("weapon_mac10",1150),Price("weapon_mp5a3",1200));Q_snprintf(teamLast,sizeof(teamLast),"4. PP-19 Bizon - $%d\n",Price("weapon_bizon",2000));}else{Q_snprintf(teamWeapons,sizeof(teamWeapons),"1. TMP - $%d\n2. MP-5 SD - $%d\n",Price("weapon_tmp",1250),Price("weapon_mp5sd",1550));Q_snprintf(teamLast,sizeof(teamLast),"4. P-90 - $%d\n",Price("weapon_p90",2350));}Q_snprintf(menu,sizeof(menu),"Submachineguns  $%d\n\n%s3. UMP .45 - $%d\n%s\n0. Exit",m_money[i],teamWeapons,Price("weapon_ump",1550),teamLast);}
	else if(page>=2&&page<=5){keys|=page==5?0x7:(page==2?0x3:1);if(page==5)Q_snprintf(menu,sizeof(menu),"Machine Guns  $%d\n\n1. M249 SAW - $%d\n2. M60 - $%d\n3. M72 LAW - $%d\n\n0. Exit",m_money[i],Price("weapon_m249",3850),Price("weapon_m60",4250),Price("weapon_m72",900));else if(page==2)Q_snprintf(menu,sizeof(menu),"Shotguns  $%d\n\n1. Shotgun - $%d\n2. XM 1014 - $%d\n\n0. Exit",m_money[i],Price("weapon_m3",1200),Price("weapon_xm1014",2000));else Q_snprintf(menu,sizeof(menu),"Submachineguns  $%d\n\n0. Exit",m_money[i]);}
	else if(page==9){keys|=1;Q_snprintf(menu,sizeof(menu),"Mines  $%d\n\n1. AP Mine - $%d\n\n0. Exit",m_money[i],Price("weapon_mineAP",500));}
...
{
	if(!CanBuy(p))return false;EnsureMoney(p);int i=p->entindex();int price=GetWeaponConfigInt(classname,"buy_price",basePrice);
...
bool CBombGameRules::BuyAmmo(CBasePlayer*p,bool primary,bool buyAll)
{
	...
	for(int slot=0;slot<MAX_ITEM_TYPES;slot++)for(CBasePlayerItem*it=p->m_rgpPlayerItems[slot];it;it=it->m_pNext)
	{
		...
		eligible++;int ammoType=p->GetAmmoIndex(w->pszAmmo1());while(true)
		{
			...
			const bool shellByShell=id==WEAPON_M3||id==WEAPON_XM1014;
			const int purchaseRounds=shellByShell?(id==WEAPON_XM1014?6:7):Q_max(1,w->iMaxClip());
			const int roundsToBuy=shellByShell?Q_min(purchaseRounds,Q_max(0,w->iMaxAmmo1()-(ammoType>=0?p->m_rgAmmo[ammoType]:0))):purchaseRounds;
			const int purchaseCost=shellByShell?roundsToBuy*cost:cost;
			...
			if(after<=before)break;spent+=shellByShell?(after-before)*cost:cost;
...
void CBombGameRules::SelectBuyMenu(CBasePlayer*p,int slot)
{
	...
	if(page==0){if(slot>=1&&slot<=5){ShowBuyMenu(p,slot);return;}if(slot==6){if(BuyAmmo(p,true)){CloseBuyMenu(p);return;}}else if(slot==7){if(BuyAmmo(p,false)){CloseBuyMenu(p);return;}}else if(slot==8||slot==9){ShowBuyMenu(p,slot);return;}}
	...
	else if(page==4&&slot==5&&!Q_stricmp(p->TeamID(),BLUE)){if(BuyWeapon(p,"weapon_sg550",WEAPON_SG550,4900)){CloseBuyMenu(p);return;}}
	else if(page==4&&slot==6){if(BuyWeapon(p,"weapon_awp",WEAPON_AWP,4750)){CloseBuyMenu(p);return;}}
	else if(page==5&&slot==1){if(BuyWeapon(p,"weapon_m249",WEAPON_M249,3850)){CloseBuyMenu(p);return;}}
	else if(page==5&&slot==2){if(BuyWeapon(p,"weapon_m60",WEAPON_M60,4250)){CloseBuyMenu(p);return;}}
	else if(page==5&&slot==3){if(BuyWeapon(p,"weapon_m72",WEAPON_M72,900)){CloseBuyMenu(p);return;}}
	...
	else if(page==2&&slot==1){if(BuyWeapon(p,"weapon_m3",WEAPON_M3,1200)){CloseBuyMenu(p);return;}}
	else if(page==2&&slot==2){if(BuyWeapon(p,"weapon_xm1014",WEAPON_XM1014,2000)){CloseBuyMenu(p);return;}}
	else if(page==9&&slot==1){if(BuyWeapon(p,"weapon_mineAP",WEAPON_MINE_AP,500)){CloseBuyMenu(p);return;}}
	else if(page==8&&BuyEquipment(p,slot)){CloseBuyMenu(p);return;}ShowBuyMenu(p,page);
}
...
bool CBombGameRules::SellWeapon(CBasePlayer*p)
{
	...
	int id=w->iWeaponID(),grenadePrice=id==WEAPON_HANDGRENADE?300:id==WEAPON_FLASHBANG?200:(id==WEAPON_GASGRENADE||id==WEAPON_SMOKEGRENADE)?300:0;
	if(grenadePrice>0){int ammo=w->PrimaryAmmoIndex();if(ammo<0||p->m_rgAmmo[ammo]<=0){HudNotice(p,"No grenade to sell");return false;}--p->m_rgAmmo[ammo];if(p->m_rgAmmo[ammo]==0){p->SelectItem("weapon_knife");p->RemoveWeapon(id);w->DestroyItem();}p->SendAmmoUpdate();AddMoney(p,grenadePrice);HudNotice(p,UTIL_VarArgs("Grenade sold for $%d",grenadePrice));return true;}
	int basePrice=0;switch(id){case WEAPON_XM1014:basePrice=2000;break;case WEAPON_AWP:basePrice=4750;break;case WEAPON_GLOCK18:basePrice=300;break;case WEAPON_BERETTA:basePrice=400;break;case WEAPON_P229:basePrice=650;break;case WEAPON_FIVESEVEN:basePrice=700;break;case WEAPON_USP:basePrice=500;break;case WEAPON_COLT1911:basePrice=400;break;case WEAPON_RAGINGBULL:basePrice=900;break;case WEAPON_DEAGLE:basePrice=800;break;case WEAPON_M3:basePrice=1200;break;case WEAPON_MP5A3:basePrice=1200;break;case WEAPON_MP5SD:basePrice=1550;break;case WEAPON_MAC10:basePrice=1150;break;case WEAPON_TMP:basePrice=1250;break;case WEAPON_UMP:basePrice=1550;break;case WEAPON_P90:basePrice=2350;break;case WEAPON_BIZON:basePrice=2000;break;case WEAPON_GALIL:basePrice=1800;break;case WEAPON_FAMAS:basePrice=1950;break;case WEAPON_SG552:basePrice=3000;break;case WEAPON_G3SG1:basePrice=4700;break;case WEAPON_SG550:basePrice=4900;break;case WEAPON_AUG:basePrice=3300;break;case WEAPON_M4:basePrice=3100;break;case WEAPON_M24:basePrice=1800;break;case WEAPON_AK47:basePrice=2700;break;case WEAPON_M60:basePrice=4250;break;default:HudNotice(p,"This weapon cannot be sold");return false;}
	int i=p->entindex();const char*classname=STRING(w->pev->classname);const bool pristine=m_weaponPurchased[i][id]&&!m_weaponFired[i][id];int value=GetWeaponConfigInt(classname,pristine?"sell_price_new":"sell_price_used",pristine?basePrice:(basePrice*60)/100);
...
void CBombGameRules::CaptureGroundWeapons(GroundWeaponSnapshot*out,int&count)
{
	...
	while(count<64&&(entity=UTIL_FindEntityByClassname(entity,"weaponbox"))!=NULL)
	{
		...
		for(int slot=0;slot<MAX_ITEM_TYPES&&s.weaponCount<MAX_WEAPONS;slot++)
			...
			{
				if(FClassnameIs(item->pev,"weapon_bomb") || FClassnameIs(item->pev,"weapon_mineAP"))continue;
				int n=s.weaponCount++;
		...
		for(int a=0;a<MAX_AMMO_SLOTS&&s.ammoCount<MAX_AMMO_SLOTS;a++)if(!FStringNull(box->m_rgiszAmmo[a])&&box->m_rgAmmo[a]>0&&Q_stricmp(STRING(box->m_rgiszAmmo[a]),"AP Mine"))
		{
			int n=s.ammoCount++;
...
void CBombGameRules::RestoreGroundWeapons(const GroundWeaponSnapshot*in,int count)
{
	...
	{
		const GroundWeaponSnapshot&s=in[i];if(!s.valid || (!s.weaponCount && !s.ammoCount))continue;
...
void CBombGameRules::SetKnifeAsLastItem(CBasePlayer*p){if(!p)return;for(int slot=0;slot<MAX_ITEM_TYPES;slot++)for(CBasePlayerItem*item=p->m_rgpPlayerItems[slot];item;item=item->m_pNext)if(FClassnameIs(item->pev,"weapon_knife")){p->m_pLastItem=item;return;}}
...
void CBombGameRules::StartRound()
{
	...
	CBaseEntity *groundM249=NULL;
	while((groundM249=UTIL_FindEntityByClassname(groundM249,"weapon_m249"))!=NULL)
	{
		CBasePlayerWeapon *weapon=dynamic_cast<CBasePlayerWeapon*>(groundM249);
		if(weapon&&!weapon->m_pPlayer)UTIL_Remove(weapon);
	}
	CBaseEntity *groundXM1014=NULL;
	while((groundXM1014=UTIL_FindEntityByClassname(groundXM1014,"weapon_xm1014"))!=NULL)
	{
		CBasePlayerWeapon *weapon=dynamic_cast<CBasePlayerWeapon*>(groundXM1014);
		if(weapon&&!weapon->m_pPlayer)UTIL_Remove(weapon);
	}
	...
	CBaseEntity *groundG3SG1=NULL;
	while((groundG3SG1=UTIL_FindEntityByClassname(groundG3SG1,"weapon_g3sg1"))!=NULL)
	{
		CBasePlayerWeapon *weapon=dynamic_cast<CBasePlayerWeapon*>(groundG3SG1);
		if(weapon&&!weapon->m_pPlayer)UTIL_Remove(weapon);
	}
	CBaseEntity *groundAUG=NULL;
	CBaseEntity *groundAWP=NULL;
	while((groundAWP=UTIL_FindEntityByClassname(groundAWP,"weapon_awp"))!=NULL)
	{
		CBasePlayerWeapon *weapon=dynamic_cast<CBasePlayerWeapon*>(groundAWP);
		if(weapon&&!weapon->m_pPlayer)UTIL_Remove(weapon);
	}
	CBaseEntity *groundSG550=NULL;
	while((groundSG550=UTIL_FindEntityByClassname(groundSG550,"weapon_sg550"))!=NULL)
	{
		CBasePlayerWeapon *weapon=dynamic_cast<CBasePlayerWeapon*>(groundSG550);
		if(weapon&&!weapon->m_pPlayer)UTIL_Remove(weapon);
	}
	...
	CBaseEntity* groundMine = NULL;
	while((groundMine=UTIL_FindEntityByClassname(groundMine,"weapon_mineAP"))!=NULL)
	{ CBasePlayerWeapon* w=dynamic_cast<CBasePlayerWeapon*>(groundMine); if(w && !w->m_pPlayer) UTIL_Remove(w); }
	const char *cleanup[]={"planted_mineAP","planted_bomb","weapon_bomb","weapon_m72","spent_m72","dropped_money","item_dropped_magazine",
	...
	const char *droppedGrenadeClasses[]={"weapon_handgrenade","weapon_flashbang","weapon_gasgrenade","weapon_smokegrenade"};
	for(int c=0;c<4;c++)
	{
	...
	for(int i=1;i<=gpGlobals->maxClients;i++)
	{
		...
		if(IsValidTeam(p->TeamID())){p->RemoveAllItems(TRUE);respawn(p,FALSE);p->SetAbsVelocity(g_vecZero);p->pev->basevelocity=g_vecZero;p->pev->avelocity=g_vecZero;p->m_flFallVelocity=0;ClearBits(p->pev->flags,FL_BASEVELOCITY|FL_FROZEN);UTIL_DropToFloor(p);p->ResetBombRoundEffects();RestoreEquipment(p,m_transitionEquipment[i]);if(!p->HasNamedPlayerItem("weapon_knife"))p->GiveNamedItem("weapon_knife");if(p->m_rgpPlayerItems[2]==NULL){const bool blue=!Q_stricmp(p->TeamID(),BLUE);p->GiveNamedItem(blue?"weapon_beretta":"weapon_glock18");m_weaponPurchased[i][blue?WEAPON_BERETTA:WEAPON_GLOCK18]=false;}p->m_bBombFreezeTime=m_freezeEnd>gpGlobals->time;g_engfuncs.pfnSetPhysicsKeyValue(p->edict(),"bombfreeze",p->m_bBombFreezeTime?"1":"0");}
