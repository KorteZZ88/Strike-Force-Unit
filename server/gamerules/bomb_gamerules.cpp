...
{
	for(int i=1;i<=gpGlobals->maxClients;i++){CBasePlayer*p=(CBasePlayer*)UTIL_PlayerByIndex(i);if(m_executingForcedRestart)m_transitionEquipment[i]=m_roundStartEquipment[i];else{CaptureEquipment(p,m_transitionEquipment[i]);if(m_diedThisRound[i]){m_transitionEquipment[i].armor=0;m_transitionEquipment[i].helmet=false;}}}
