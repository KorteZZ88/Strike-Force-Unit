...
void CBombGameRules::SelectBuyMenu(CBasePlayer*p,int slot)
{
	...
	else if(page==1&&(slot>=1&&slot<=4)){bool bought=false;if(slot==1)bought=BuyWeapon(p,"weapon_glock18",WEAPON_GLOCK18,300);else if(slot==2)bought=BuyWeapon(p,"weapon_beretta",WEAPON_BERETTA,400);else if(slot==3){if(!Q_stricmp(p->TeamID(),RED))bought=BuyWeapon(p,"weapon_1911",WEAPON_COLT1911,400);else if(!Q_stricmp(p->TeamID(),BLUE))bought=BuyWeapon(p,"weapon_usp",WEAPON_USP,500);}else bought=BuyWeapon(p,"weapon_rbull",WEAPON_RBULL,750);if(bought){CloseBuyMenu(p);return;}}
...
{
	for(int i=1;i<=gpGlobals->maxClients;i++){CBasePlayer*p=(CBasePlayer*)UTIL_PlayerByIndex(i);if(m_executingForcedRestart)m_transitionEquipment[i]=m_roundStartEquipment[i];else{CaptureEquipment(p,m_transitionEquipment[i]);if(m_diedThisRound[i])m_transitionEquipment[i].armor=0;}}
