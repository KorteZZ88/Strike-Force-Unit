...
DEFINE_FIELD(m_iszEndTarget, FIELD_STRING),
DEFINE_FIELD(m_iEndTargetMode, FIELD_INTEGER),
DEFINE_FIELD(m_hEndTarget, FIELD_EHANDLE),
DEFINE_FIELD(m_flAttachedObjectsOffset, FIELD_FLOAT),
DEFINE_FIELD(m_bMakeSound, FIELD_BOOLEAN),
DEFINE_FIELD(m_vecEndTargetBaseAngles, FIELD_VECTOR),
DEFINE_FIELD(m_bEndTargetAnglesInitialized, FIELD_BOOLEAN),
END_DATADESC()
...
CRope::CRope()
{
	...
	m_iEndTargetMode = 0;
	m_bEndTargetAnglesInitialized = false;
}
...
void CRope::KeyValue(KeyValueData* pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "endtarget"))
	{
		m_iszEndTarget = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = true;
	}
	else if (FStrEq(pkvd->szKeyName, "endtargetmode"))
	{
		m_iEndTargetMode = atoi(pkvd->szValue);
...
void CRope::UpdateEndTarget(void)
{
	...
	{
		m_hEndTarget = UTIL_FindEntityByTargetname(
			NULL,
			STRING(m_iszEndTarget));
	...
	// Один раз запоминаем углы, выставленные в JACK.
	...
	// Объект всегда следует за концом верёвки.
	pEndTarget->SetAbsOrigin(m_vecLastEndPos);

	// Режим 0: жёстко сохраняем исходную ориентацию.
	if (m_iEndTargetMode == 0)
	{
	...
	CRopeSegment *pLastSample =
		m_pSegments[m_iNumSamples - 1];

	CRopeSegment *pPreviousSample =
		m_pSegments[m_iNumSamples - 2];

	if (pLastSample == NULL || pPreviousSample == NULL)
		return;

	Vector vecDirection =
		pLastSample->m_Data.mPosition -
	...
	{
		// Используем исходный yaw объекта.
		// Благодаря этому он не переворачивается на 180 градусов.
		...
		float flForward =
			DotProduct(vecDirection, vecForward);

		float flRight =
			DotProduct(vecDirection, vecRight);

		float flDown = -vecDirection.z;

		// Защита от почти горизонтального положения.
		if (flDown < 0.01f)
			flDown = 0.01f;

		float flPitch =
			RAD2DEG(atan2(flForward, flDown));

		float flRoll =
			RAD2DEG(atan2(flRight, flDown));

		Vector vecAngles = m_vecEndTargetBaseAngles;
...
void CRope::ApplyExplosionForce(const Vector& vecExplosionOrigin, const float flRadius, const float flForce)
{
	if (flRadius <= 0.0f || flForce <= 0.0f)
		return;
	...
	for (int i = 1; i < m_iNumSamples; i++)
	{
		...
		Vector vecDelta = pSegment->m_Data.mPosition - vecExplosionOrigin;
		...
		if (flDistance > 0.01f)
			vecDirection = vecDelta / flDistance;
		else
			vecDirection = Vector(0.0f, 0.0f, 1.0f);

		float flFalloff = 1.0f - (flDistance / flRadius);

		vecDirection.z += 0.15f;
		vecDirection = vecDirection.Normalize();

		pSegment->ApplyExternalForce(vecDirection * flForce * flFalloff);
	}
}

void UTIL_ApplyExplosionForceToRopes(const Vector& vecExplosionOrigin, const float flRadius, const float flForce)
{
	CBaseEntity *pEntity = NULL;

	while ((pEntity = UTIL_FindEntityByClassname(pEntity, "env_rope")) != NULL)
	{
		CRope *pRope = static_cast<CRope *>(pEntity);

		if (pRope != NULL)
			pRope->ApplyExplosionForce(vecExplosionOrigin, flRadius, flForce);
	}
}
