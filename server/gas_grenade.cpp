...
BEGIN_DATADESC(CGasGrenade) DEFINE_FIELD(m_flGasEnd,FIELD_TIME), DEFINE_FIELD(m_flModelEnd,FIELD_TIME), DEFINE_FIELD(m_flNextCloud,FIELD_TIME), DEFINE_FUNCTION(DetonateGas), DEFINE_FUNCTION(GasThink), END_DATADESC()
CGasGrenade* CGasGrenade::ShootTimed(entvars_t* owner,Vector start,Vector velocity,float time) { CGasGrenade* g=GetClassPtr((CGasGrenade*)NULL); g_iPoisonSprite=MODEL_INDEX("sprites/poison.spr"); g->Spawn(); g->pev->classname=MAKE_STRING("gas_grenade"); SET_MODEL(g->edict(),"models/weapon/Gasgrenade/w_smokegrenade.mdl"); UTIL_SetOrigin(g,start); g->SetAbsVelocity(velocity); g->pev->owner=ENT(owner); g->pev->gravity=.5f; g->pev->friction=.8f; g->SetTouch(&CGrenade::BounceTouch); g->SetThink(&CGasGrenade::DetonateGas); g->SetNextThink(time); return g; }
void CGasGrenade::BounceSound() { EMIT_SOUND(edict(),CHAN_BODY,"weapons/flashbang/flashbang_hit.wav",.7f,ATTN_NORM); }
void CGasGrenade::DetonateGas() { SetAbsVelocity(g_vecZero); pev->solid=SOLID_NOT; m_flGasEnd=gpGlobals->time+20.f; m_flModelEnd=gpGlobals->time+18.f; m_flNextCloud=0; SetThink(&CGasGrenade::GasThink); SetNextThink(0); }
void CGasGrenade::GasThink()
{
	const Vector o=GetAbsOrigin();
	if (gpGlobals->time>=m_flGasEnd) { UTIL_Remove(this); return; }
	if (gpGlobals->time>=m_flModelEnd) pev->effects|=EF_NODRAW;
