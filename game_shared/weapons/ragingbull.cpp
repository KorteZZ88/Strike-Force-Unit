...
#define RAGINGBULL_STANDING_SPREAD 0.00873f
#define RAGINGBULL_MOVING_SPREAD 0.025f

namespace
{
constexpr float RAGINGBULL_FIRE_INTERVAL = 60.0f / 70.0f;
}

CRagingBullWeaponContext::CRagingBullWeaponContext(std::unique_ptr<IWeaponLayer>&& layer) :
	CBaseWeaponContext(std::move(layer))
{
	m_iId = WEAPON_RAGINGBULL;
	m_iDefaultAmmo = RAGINGBULL_DEFAULT_GIVE;
	m_fInZoom = false;
	m_usFireRBull = m_pLayer->PrecacheEvent("events/ragingbull.sc");
}

int CRagingBullWeaponContext::GetItemInfo(ItemInfo *p) const
{
	p->pszName = CLASSNAME_STR(RAGINGBULL_CLASSNAME);
	p->pszAmmo1 = "357";
	p->iMaxAmmo1 = _357_MAX_CARRY;
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = RAGINGBULL_MAX_CLIP;
	p->iFlags = 0;
	p->iSlot = 1;
	p->iPosition = 6;
	p->iId = m_iId;
	p->iWeight = RAGINGBULL_WEIGHT;
	return 1;
}

bool CRagingBullWeaponContext::Deploy()
{
	// Strike Force Unit's RBull does not use laser-sight geometry.
	m_pLayer->SetWeaponBodygroup(0);

	return DefaultDeploy("models/weapon/RBull/v_rbull.mdl", "models/weapon/RBull/p_rbull.mdl", RAGINGBULL_DRAW, "python");
}

void CRagingBullWeaponContext::Holster()
{
...
void CRagingBullWeaponContext::SecondaryAttack()
{
	// Raging Bull has no alternate fire in Strike Force Unit.
	m_fInZoom = false;
}

void CRagingBullWeaponContext::PrimaryAttack()
{
...
void CRagingBullWeaponContext::Reload()
{
	...
	if (DefaultReload(RAGINGBULL_MAX_CLIP, RAGINGBULL_RELOAD, 94.0f / 34.0f, 0))
	{
#ifndef CLIENT_DLL
...
void CRagingBullWeaponContext::WeaponIdle()
{
	ResetEmptySound();
	...
	SendWeaponAnim(RAGINGBULL_IDLE1, 0);
}
