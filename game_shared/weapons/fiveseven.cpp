...
void CFiveSevenWeaponContext::PrimaryAttack()
{
	...
	m_flNextPrimaryAttack = GetNextPrimaryAttackDelay(ConfigFireInterval(60.0f / 400.0f));
...
void CFiveSevenWeaponContext::Reload()
{
	...
	if (DefaultReload(reloadClipSize, FIVESEVEN_RELOAD, ConfigValue("reload_time", 1.8f))) m_flCs16PistolAccuracy = -1.0f;
}
