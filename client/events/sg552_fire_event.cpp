...
CSG552FireEvent::CSG552FireEvent(event_args_t* args) : CBaseGameEvent(args) {}
void DlightFlash(const Vector& origin, int index);
void CSG552FireEvent::Execute()
{
	DlightFlash(GetOrigin(), GetEntityIndex());
	if (IsEventLocal()) { GameEventUtils::SpawnMuzzleflash(); gEngfuncs.pEventAPI->EV_WeaponAnimation(SG552_SHOOT1 + gEngfuncs.pfnRandomLong(0, 2), 0); }
	...
	const char* sound = gEngfuncs.pfnRandomLong(0, 1) ? "weapons/SG552/sg552-1.wav" : "weapons/SG552/sg552-2.wav";
