...
CBerettaFireEvent::CBerettaFireEvent(event_args_t *args) :
	CBaseGameEvent(args)
{
}
...
void CBerettaFireEvent::Execute()
{
	...
	if (IsEventLocal())
	{
		...
		gEngfuncs.pEventAPI->EV_WeaponAnimation(ClipEmpty() ? BERETTA_SHOOT_EMPTY : BERETTA_SHOOT, 0);
		// V_PunchAxis( 0, -2.0 );
	}
	...
	gEngfuncs.pEventAPI->EV_PlaySound(GetEntityIndex(), GetOrigin(), CHAN_WEAPON,
		"weapons/Beretta/Beretta-1.wav",
		GetGunshotVolume(), GetGunshotAttenuation(), 0, 98 + gEngfuncs.pfnRandomLong(0, 3));
}

bool CBerettaFireEvent::ClipEmpty() const
{
	return m_arguments->bparam1 != 0;
}

Vector CBerettaFireEvent::GetShootDirection(const matrix3x3 &camera) const
{
