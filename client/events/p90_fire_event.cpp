...
CP90FireEvent::CP90FireEvent(event_args_t *args) : CBaseGameEvent(args) {}

void CP90FireEvent::Execute()
{
	if (IsEventLocal())
		gEngfuncs.pEventAPI->EV_WeaponAnimation(P90_FIRE1 + gEngfuncs.pfnRandomLong(0, 2), 2);
	...
	gEngfuncs.pEventAPI->EV_PlaySound(GetEntityIndex(), GetOrigin(), CHAN_WEAPON,
		"weapons/P90/p90-1.wav", GetGunshotVolume(), ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 15));
}

Vector CP90FireEvent::GetShootDirection(const matrix3x3 &camera) const
{
