...
void CUMPFireEvent::Execute()
{
	if (IsEventLocal())
	{
		GameEventUtils::SpawnMuzzleflash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(UMP_FIRE1 + gEngfuncs.pfnRandomLong(0, 2), 2);
	}
	...
	// The event is broadcast (NotHost), so nearby clients hear this world sound.
	gEngfuncs.pEventAPI->EV_PlaySound(GetEntityIndex(), GetOrigin(), CHAN_WEAPON,
		"weapons/UMP/ump45-1.wav", GetGunshotVolume(), ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 15));
}
