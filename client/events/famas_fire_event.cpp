...
CFamasFireEvent::CFamasFireEvent(event_args_t* args) : CBaseGameEvent(args) {}
void DlightFlash(const Vector& origin, int index);
void CFamasFireEvent::Execute()
{
	DlightFlash(GetOrigin(), GetEntityIndex());
	if (IsEventLocal()) { GameEventUtils::SpawnMuzzleflash(); gEngfuncs.pEventAPI->EV_WeaponAnimation(FAMAS_SHOOT1 + gEngfuncs.pfnRandomLong(0, 2), 0); }
	...
	const char* sound = gEngfuncs.pfnRandomLong(0, 1) ? "weapons/Famas/famas-1.wav" : "weapons/Famas/famas-2.wav";
