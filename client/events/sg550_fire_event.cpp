...
CSG550FireEvent::CSG550FireEvent(event_args_t* args) : CBaseGameEvent(args) {}
void DlightFlash(const Vector& origin, int index);
void CSG550FireEvent::Execute()
{
	...
	gEngfuncs.pEventAPI->EV_PlaySound(GetEntityIndex(), GetOrigin(), CHAN_WEAPON, "weapons/SG550/sg550-1.wav", GetGunshotVolume(), GetGunshotAttenuation(), 0, 94+gEngfuncs.pfnRandomLong(0,15));
}
