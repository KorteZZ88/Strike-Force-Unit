...
static_assert(RAGINGBULL_FIRE1 == 1, "RBull shoot1 must remain sequence 1");

CRagingBullFireEvent::CRagingBullFireEvent(event_args_t *args) :
	CBaseGameEvent(args)
{
}
...
void CRagingBullFireEvent::Execute()
{
	...
	if (IsEventLocal())
	{
		...
		gEngfuncs.pEventAPI->EV_WeaponAnimation( RAGINGBULL_FIRE1, 0 );
		// V_PunchAxis( 0, -10.0 );
	}
...
Vector CRagingBullFireEvent::GetShootDirection(const matrix3x3 &camera) const
{
